...
import { FILTER_PRESETS, getFilterAdjustments } from '../filters';
import { FRAMES } from '../frames';
import { loadImage, fileToDataURL } from '../canvasUtils';
import { getCanvasPos, strokeBetween, eraseStroke, cloneStroke, healStroke } from '../brushUtils';
...
export default function Editor({ imageSrc, onBack }: EditorProps) {
  ...
  const repairSourceRef = useRef<{ x: number; y: number } | null>(null);
  ...
  const [repairHasSource, setRepairHasSource] = useState(false);

  // Background erase state
  const [bgBrushSize, setBgBrushSize] = useState(40);
  const [bgHardness, setBgHardness] = useState(60);
  const [bgZoom, setBgZoom] = useState(1);
  const [bgTool, setBgTool] = useState<'erase' | 'restore'>('erase');
  const [bgMode, setBgMode] = useState<'transparent' | 'solid' | 'image'>('transparent');
  const [bgColor, setBgColor] = useState('#ffffff');
  const bgEraseCanvasRef = useRef<HTMLCanvasElement | null>(null);
  const bgDisplayRef = useRef<HTMLCanvasElement | null>(null);
  const bgDrawingRef = useRef(false);
  const [bgUndoStack, setBgUndoStack] = useState<ImageData[]>([]);
  const [bgRedoStack, setBgRedoStack] = useState<ImageData[]>([]);
  ...
  const pushHistory = useCallback(() => {
    ...
    const state: HistoryState = {
      ...
      bgEraseDataUrl: bgEraseCanvasRef.current?.toDataURL() || null,
  ...
  const restoreState = useCallback((state: HistoryState) => {
    ...
    if (state.bgEraseDataUrl && bgEraseCanvasRef.current) {
      loadImage(state.bgEraseDataUrl).then((img) => {
        const ctx = bgEraseCanvasRef.current!.getContext('2d')!;
        ctx.clearRect(0, 0, bgEraseCanvasRef.current!.width, bgEraseCanvasRef.current!.height);
        ctx.drawImage(img, 0, 0);
      });
    }
  ...
  const initBgCanvas = useCallback(() => {
    if (!image || bgEraseCanvasRef.current) return;
    const canvas = document.createElement('canvas');
    const maxDim = 2048;
    let w = image.naturalWidth, h = image.naturalHeight;
    if (w > maxDim || h > maxDim) {
    ...
    canvas.width = w; canvas.height = h;
    canvas.getContext('2d')!.drawImage(image, 0, 0, w, h);
    bgEraseCanvasRef.current = canvas;
  }, [image]);
  ...
  const handleToolSelect = (tool: ToolId) => {
    ...
    if (tool === 'background') initBgCanvas();
  ...
  // ── Repair handlers (automatic healing) ──────────────────
  ...
  const handleBgPointerDown = (e: React.PointerEvent) => {
    if (!bgEraseCanvasRef.current || activeTool !== 'background') return;
    const canvas = bgEraseCanvasRef.current;
    const pos = getCanvasPos(e, canvas);
    pushHistory();
    bgDrawingRef.current = true;
    const ctx = canvas.getContext('2d')!;
    const snapshot = ctx.getImageData(0, 0, canvas.width, canvas.height);
    setBgUndoStack((prev) => [...prev.slice(-9), snapshot]);
    if (bgTool === 'erase') {
      eraseStroke(ctx, pos, pos, bgBrushSize, bgHardness / 100);
    } else {
      ctx.save();
      ctx.globalCompositeOperation = 'source-over';
      ctx.globalAlpha = 1;
      ctx.beginPath();
      ctx.arc(pos.x, pos.y, bgBrushSize, 0, Math.PI * 2);
      ctx.clip();
  ...
  const handleBgPointerMove = (e: React.PointerEvent) => {
    if (!bgDrawingRef.current || !bgEraseCanvasRef.current) return;
    const canvas = bgEraseCanvasRef.current;
    const pos = getCanvasPos(e, canvas);
    const ctx = canvas.getContext('2d')!;
    const lastPos = (canvas as any)._lastPos || pos;
    if (bgTool === 'erase') {
      eraseStroke(ctx, lastPos, pos, bgBrushSize, bgHardness / 100);
    } else {
      ctx.save();
      ctx.globalCompositeOperation = 'source-over';
      ctx.beginPath();
    ...
    setBgVersion((v) => v + 1);
    (canvas as any)._lastPos = pos;
  };

  const handleBgPointerUp = () => {
    bgDrawingRef.current = false;
    if (bgEraseCanvasRef.current) (bgEraseCanvasRef.current as any)._lastPos = null;
  };

  const bgUndo = () => {
    setBgUndoStack((prev) => {
      if (prev.length === 0 || !bgEraseCanvasRef.current) return prev;
      const ctx = bgEraseCanvasRef.current.getContext('2d')!;
      const current = ctx.getImageData(0, 0, bgEraseCanvasRef.current.width, bgEraseCanvasRef.current.height);
      setBgRedoStack((r) => [...r, current]);
      ctx.putImageData(prev[prev.length - 1], 0, 0);
      setBgVersion((v) => v + 1);
      return prev.slice(0, -1);
    });
  ...
  const bgClear = () => {
    if (!bgEraseCanvasRef.current) return;
    pushHistory();
    const ctx = bgEraseCanvasRef.current.getContext('2d')!;
    const snapshot = ctx.getImageData(0, 0, bgEraseCanvasRef.current.width, bgEraseCanvasRef.current.height);
    setBgUndoStack((prev) => [...prev.slice(-9), snapshot]);
    ctx.clearRect(0, 0, bgEraseCanvasRef.current.width, bgEraseCanvasRef.current.height);
    setBgVersion((v) => v + 1);
  };
  ...
  const bgReset = () => {
    if (!bgEraseCanvasRef.current || !image) return;
    pushHistory();
    const ctx = bgEraseCanvasRef.current.getContext('2d')!;
    const snapshot = ctx.getImageData(0, 0, bgEraseCanvasRef.current.width, bgEraseCanvasRef.current.height);
    setBgUndoStack((prev) => [...prev.slice(-9), snapshot]);
    ctx.clearRect(0, 0, bgEraseCanvasRef.current.width, bgEraseCanvasRef.current.height);
    ctx.drawImage(image, 0, 0, bgEraseCanvasRef.current.width, bgEraseCanvasRef.current.height);
    setBgRedoStack([]);
    setBgVersion((v) => v + 1);
  };
  ...
  const applyBrush = (ctx: CanvasRenderingContext2D, from: { x: number; y: number }, to: { x: number; y: number }, canvas: HTMLCanvasElement) => {
    if (brushTool === 'eraser') {
      ...
      // Blur brush: copy region, blur it, draw back
      const r = brushSize;
      ctx.save();
  ...
  // Inpainting: fill masked area with surrounding pixels
  const objEraserApply = useCallback(() => {
    ...
    // Simple inpainting: for each masked pixel, sample surrounding non-masked pixels
    const sampleRadius = Math.max(5, objEraserBrushSize);
    for (let y = 0; y < canvas.height; y++) {
      for (let x = 0; x < canvas.width; x++) {
        const idx = (y * canvas.width + x) * 4;
        if (mData[idx + 3] > 50) {
          // This pixel is masked — sample surrounding
  ...
  const addText = () => {
    pushHistory();
    const maxDim = 2048;
    let imgW = image.naturalWidth, imgH = image.naturalHeight;
  ...
  // Render bg display canvas
  useEffect(() => {
    if (activeTool !== 'background' || !bgDisplayRef.current || !bgEraseCanvasRef.current) return;
    const display = bgDisplayRef.current;
    const src = bgEraseCanvasRef.current;
    display.width = src.width; display.height = src.height;
    const ctx = display.getContext('2d')!;
    ctx.clearRect(0, 0, display.width, display.height);
    ctx.drawImage(src, 0, 0);
  }, [activeTool, bgVersion]);

  // Render object eraser display canvas (image + mask overlay)
  useEffect(() => {
  ...
  const isObjEraserMode = activeTool === 'objectEraser';

  // Determine which draw handlers to use
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-neutral-950">
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden min-h-0">
        ...
        ) : activeTool === 'background' && bgEraseCanvasRef.current ? (
          <div className="flex-1 flex items-center justify-center overflow-hidden p-4">
            <div className="relative">
              <div className="checkerboard absolute inset-0 rounded-lg" />
              <canvas
                ref={bgDisplayRef}
                onPointerDown={handleBgPointerDown}
                onPointerMove={handleBgPointerMove}
                onPointerUp={handleBgPointerUp}
                className="relative max-w-full max-h-full object-contain touch-none rounded-lg"
                style={{ touchAction: 'none', transform: `scale(${bgZoom})`, transformOrigin: 'center' }}
              />
            </div>
          </div>
      ...
      {/* Tool panel — replaces the main toolbar when a tool is active */}
      {activeTool && activeTool !== 'crop' && (
        <div className="bg-neutral-900/95 backdrop-blur border-t border-neutral-800 px-4 py-3 max-h-[40vh] overflow-y-auto no-scrollbar animate-slide-up safe-bottom shrink-0">
          ...
          {activeTool === 'background' && (
            <BackgroundPanel
              brushSize={bgBrushSize} hardness={bgHardness} zoom={bgZoom} tool={bgTool}
              onBrushSize={setBgBrushSize} onHardness={setBgHardness} onZoom={setBgZoom} onTool={setBgTool}
              onUndo={bgUndo} onRedo={bgRedo} onClear={bgClear} onReset={bgReset}
              canUndo={bgUndoStack.length > 0} canRedo={bgRedoStack.length > 0}
              bgMode={bgMode} bgColor={bgColor} onBgMode={setBgMode} onBgColor={setBgColor}
              onBgImage={() => bgImageRef.current?.click()}
            />
          )}
      ...
      <input ref={bgImageRef} type="file" accept="image/jpeg,image/jpg,image/png,image/webp" className="hidden" onChange={async (e) => {
        const file = e.target.files?.[0];
        if (!file) return;
        const dataUrl = await fileToDataURL(file);
        const img = await loadImage(dataUrl);
        // Handle bg image for background panel
        e.target.value = '';
      }} />
