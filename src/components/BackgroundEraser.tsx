...
export default function BackgroundEraser({ onBack }: Props) {
  ...
  const renderDisplay = useCallback(() => {
    ...
    // Keep pixels where mask is opaque,
    // remove pixels where mask is transparent.
  ...
  const initImage = useCallback(async (file: File) => {
    ...
    // Fully opaque mask = keep original image.
  ...
  // Set display canvas size once it is in the DOM.
  useEffect(() => {
    if (
      stage === 'edit' &&
      imgSize.w > 0 &&
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (activeTool === 'magic') {
        // Flood fill is handled by the magic tap handler.
        return;
      }
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      if (
        ...
        setPan({
          x:
            panStartRef.current.px +
            dx,
          y:
            panStartRef.current.py +
            dy,
        });
  ...
  const handleAuto = useCallback(
    async () => {
      ...
      try {
        ...
        const result = await removeBg(
          ...
          (
            ...
            if (isDownload) {
              ...
              setProgressLabel(
                `Downloading AI Model (20MB one-time) ${Math.round(
                  pct * 100
                )}%`
              );
            } else if (
        ...
        // Phase 2: Apply the AI alpha mask.
        const resultImg =
        ...
        if (mask) {
          ...
          // Draw AI result at full mask resolution.
          const tempCanvas =
          ...
          // Safety check:
          // never replace the whole photo with an empty mask.
          if (visiblePixels === 0) {
            ctx.fillStyle =
              'rgba(0,0,0,1)';
        ...
        // If fallback mode was used, show the warning.
        if (result.method === 'fallback') {
          setTimeout(() => {
            setErrorMsg(
              'AI model is blocked in this preview environment. The basic color-difference method was used instead. For perfect results, use the Manual Eraser tools, or build the APK where the full AI model runs 100% offline.'
            );
        ...
        if (
          msg.includes('transferable') ||
          msg.includes('Worker') ||
          msg.includes('postMessage')
        ) {
          setErrorMsg(
            'Preview me Worker block hai, APK me 100% chalega. For testing use Manual Eraser.'
          );
        } else {
          setErrorMsg(msg);
        }
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-6 animate-fade-in">
            ...
            <div className="text-center max-w-xs">
              ...
              <p className="text-neutral-400 text-sm leading-relaxed">
                100% offline AI removes
                backgrounds from any photo
                — people, products, animals,
                objects. Manual touch-up
                tools included. Export as
                transparent PNG.
              </p>
            </div>
            ...
            <div className="shrink-0 pt-2 space-y-2">
              ...
              {activeTool === 'zoom' && (
                <div className="flex items-center gap-2 px-1">
                  <button
                    onClick={zoomOut}
                    className="w-9 h-9 flex items-center justify-center rounded-xl bg-neutral-800 hover:bg-neutral-700 active:scale-95 transition-all shrink-0"
                  >
            ...
            {aiPhase ===
              ...
              <div className="text-center max-w-xs">
                <p className="text-xs text-neutral-500">
                  One-time download for
                  offline use. After this,
                  Auto works without
                  internet.
                </p>
